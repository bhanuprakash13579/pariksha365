...
async def create_course(db: AsyncSession, course_in: CourseCreate) -> Course:
    ...
    from sqlalchemy.orm import selectinload
    stmt = select(Course).options(selectinload(Course.folders)).where(Course.id == db_course.id).execution_options(populate_existing=True)
...
async def create_course_folder(db: AsyncSession, course_id: uuid.UUID, folder_in: CourseFolderCreate) -> CourseFolder:
    ...
    from sqlalchemy.orm import selectinload
    stmt = select(CourseFolder).where(CourseFolder.id == db_folder.id).options(selectinload(CourseFolder.tests)).execution_options(populate_existing=True)
    result = await db.execute(stmt)
    return result.scalars().first()
