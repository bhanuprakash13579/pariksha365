---
title: "History — The Complete Study Book"
subtitle: "Indian + World History, exam-first, memory-first"
author: "Pariksha365"
version: "2026-04"
documentclass: book
---
...
```mermaid
graph LR
    IVC[Major IVC cities]
    IVC --> A[Harappa<br/>Punjab Pak, on Ravi]
    IVC --> B[Mohenjo-daro<br/>Sindh Pak, on Indus]
    IVC --> C[Dholavira<br/>Gujarat IN, Khadir Bet]
    IVC --> D[Rakhigarhi<br/>Haryana IN, on Ghaggar]
    IVC --> E[Ganweriwala<br/>Punjab Pak, on Ghaggar-Hakra]
    IVC --> F[Kalibangan<br/>Rajasthan IN, on Ghaggar]
```
...
```mermaid
graph LR
    P[Lothal + Sutkagendor + Allahdino]
    P -- carnelian, cotton, ivory, beads, copper --> M[Mesopotamia — Sumer, Akkad]
    P -- timber, spices, cotton --> E[Egypt indirect]
    P -- lapis + tin --> A[Afghanistan]
    P -- copper --> R[Khetri, Rajasthan]
    P -- gold --> K[Karnataka]
    P -- shells --> SA[Saurashtra]
```
...
```mermaid
graph TD
    V[Vedas<br/>shruti — heard]
    V --> R[Rig Veda<br/>1028 hymns, 10 Mandalas<br/>oldest, pre-1000 BCE]
    V --> S[Sama Veda<br/>melodies — music]
    V --> Y[Yajur Veda<br/>sacrificial formulae<br/>Shukla + Krishna variants]
    V --> A[Atharva Veda<br/>magic + healing + folk]
    R --> B1[Brahmanas<br/>ritual commentary]
    R --> B2[Aranyakas<br/>forest meditations]
    R --> B3[Upanishads<br/>philosophy]
```
...
```mermaid
graph LR
    H[Haryanka<br/>Bimbisara → Ajatashatru → Udayin<br/>moved capital to Pataliputra]
    H --> S[Shishunaga<br/>Shishunaga → Kalashoka<br/>Buddhist 2nd Council at Vaishali]
    S --> N[Nanda<br/>first non-Kshatriya dynasty<br/>Mahapadma Nanda → Dhana Nanda]
    N --> M[Maurya<br/>Chandragupta — Ashoka]
```
...
graph TD
    EP[Eightfold Path]
    EP --> W1[Right View]
    EP --> W2[Right Intention]
    EP --> W3[Right Speech]
    EP --> W4[Right Action]
    EP --> W5[Right Livelihood]
    EP --> W6[Right Effort]
    EP --> W7[Right Mindfulness]
    EP --> W8[Right Concentration]
```
...
graph TD
    ...
    C1 --> C2[2nd Council<br/>Vaishali 383 BCE<br/>Kalashoka<br/>Sabakami chair<br/>first schism — Sthaviras vs Mahasanghikas]
    C2 --> C3[3rd Council<br/>Pataliputra 250 BCE<br/>Ashoka<br/>Moggaliputta Tissa chair<br/>Abhidhamma Pitaka added; missions dispatched]
    C3 --> C4[4th Council<br/>Kundalavana Kashmir 1st cent CE<br/>Kanishka<br/>Vasumitra chair + Ashvaghosha<br/>Mahayana crystallises]
```
...
timeline
    ...
    3300 BCE : Early Harappan begins (Mehrgarh continuity).
    2600 BCE : Mature Harappan — city phase.
    1900 BCE : Late Harappan — de-urbanisation.
    1500 BCE : Early Vedic begins (Rig Veda).
    1000 BCE : Later Vedic — iron + Gangetic plain.
    600 BCE : 16 Mahajanapadas — Magadha rises.
    563 BCE : Gautama Buddha born (traditional).
    540 BCE : Mahavira born (traditional).
    483 BCE : Buddha's Mahaparinirvana — 1st Buddhist Council.
    322 BCE : Chandragupta Maurya founds Mauryan empire.
    305 BCE : Defeats Seleucus Nicator.
    268 BCE : Ashoka begins reign.
    261 BCE : Kalinga War.
    250 BCE : 3rd Buddhist Council — Ashokan missions.
    185 BCE : Pushyamitra Sunga kills Brihadratha — end of Mauryas.
    78 CE : Saka Era begins (Kanishka).
    319 CE : Gupta Era begins (Chandragupta I).
    400 CE : Fa-Hien visits under Chandragupta II.
    476 CE : Aryabhata's Aryabhatiya.
    606 CE : Harshavardhana begins reign.
    630 CE : Hiuen Tsang arrives; Pulakesin II defeats Harsha.
    985 CE : Rajaraja Chola I begins reign.
    1025 CE : Rajendra Chola naval expedition to Sri Vijaya.
```
...
```mermaid
graph TD
    Mag[Magadha 600 BCE on]
    Mag --> Hy[Haryanka — Bimbisara, Ajatashatru]
    Mag --> Sh[Shishunaga]
    Mag --> Na[Nanda]
    Mag --> Mo[Maurya — Chandragupta, Bindusara, Ashoka]
    Mo --> Su[Sunga — Pushyamitra]
    Su --> Kn[Kanva]
    Kn --> Sa[Satavahanas]
    NW[NW invasions]
    NW --> IG[Indo-Greeks — Menander]
    NW --> Sc[Shakas — Rudradaman]
    NW --> PA[Parthians]
    NW --> Ku[Kushanas — Kanishka]
    Gu[Gupta — Chandragupta I, Samudragupta, Chandragupta II]
    Gu --> PV[Pushyabhuti — Harsha]
    S[South]
    S --> Sg[Sangam — Cheras, Cholas, Pandyas]
    S --> PL[Pallavas]
    S --> Ch[Chalukyas of Badami]
    S --> Rk[Rashtrakutas]
    S --> MC[Medieval Cholas — Rajaraja, Rajendra]
```
