...
**Q.** A cube of side 5 cm is painted on all faces, then cut into 1 cm³ smaller cubes. How many small cubes have exactly 2 faces painted?

**Method.** Edge cubes (excluding corners): 12 · (n − 2) where n = 5 → 12 · 3 = **36**.

> Memorise: corners 8 (3-painted), edges 12(n−2) (2-painted), face centres 6(n−2)² (1-painted), interior (n−2)³ (0-painted).
...
Banks Mains often asks "All A being C is a possibility" — the candidate must check if at least ONE valid Venn diagram allows the universal A⊆C while satisfying the given statements.

**Q.** Statements: Some pens are pencils. All pencils are erasers.
Conclusion: All pens being erasers is a possibility.
...
Banks Mains gives 2 statements with codes, then 4 conclusions. Methods:
1. Decode each statement separately.
2. For each conclusion, check both statements for a chain proof.
3. Use "either–or" — sometimes conclusions I and II share a common element where exactly one must be true (e.g., if I says "P > Q" and II says "P = Q", and the chain doesn't decide, then "either I or II" is the answer).

## 15.6 Data sufficiency — 3-statement variant

Some banks (RBI, IBPS PO Mains) ask 3-statement DS:
- (a) Only I and II.
- (b) Only II and III.
- (c) All three.
- (d) Any two.
...
- Reversed: ESROH.
- Mirror each: E → Ǝ; S → 2 (mirror); R → Я; O → O; H → H.
- Result: **ƎƧЯOH** (reading left-to-right in mirror).
...
> **Δ (delta)** = first difference between consecutive terms.
> **Δ² (delta-squared)** = second difference (difference of Δ values). Constant Δ → arithmetic progression; constant Δ² → quadratic series.
> **Alphabet position:** A=1, B=2, …, Z=26. Memorise: M=13, N=14, O=15, P=16.
...
- Differences: 8−3=5; 15−8=7; 24−15=9 → Δ = 5, 7, 9, ?
- Δ-of-Δ (Δ²) = 2, 2 → constant. So next Δ = 9 + 2 = **11**.
- Next term = 24 + 11 = **35**.

**Worth knowing:** When Δ values increase in a constant way (here +2 each time), the underlying series is **n² + something**. Check: terms = 3, 8, 15, 24 → these are n² + (n−1) for n = 2, 3, 4, 5. Wait: 4+(-1)=3 ✓; 9+(-1)=8 ✓; verify n² − 1 for n=2,3,4,5 → 3, 8, 15, 24 ✓. So pattern is n² − 1 for n=2,3,4,5,6 → 6th = 36 − 1 = **35** ✓.

***Watch out:*** "33" — students extrapolate Δ as 5, 7, 9, 11 ✓ but compute 24 + 9 = 33 by mistake (using current Δ instead of next Δ).

**A similar question — Square minus 1.** 0, 3, 8, 15, 24, 35, ? → next term: n=7 → 49 − 1 = **48**.

**Another twist — Cubic series.** 1, 8, 27, 64, ? → cubes; next n=5 → **125**.
...
- Pattern: nth term = n(n+1)
- 6th term = 6×7 = **42**

**Quick way (if pattern not spotted).** Differences: 4, 6, 8, 10, ?. Δ² = 2 (constant). Next Δ = 12. Next term = 30 + 12 = **42** ✓.

***Watch out:*** "40" — students assume Δ keeps growing by 4 (4, 6, 8, 10, 12 looks right) but they apply 30 + 10 = 40 instead of 30 + 12.

**A similar question — Triangular numbers.** 1, 3, 6, 10, 15, 21, ? → these are n(n+1)/2. Next = 7×8/2 = **28**.

**Another twist — n²+n+1.** 3, 7, 13, 21, 31, ? → Δ = 4, 6, 8, 10. Next Δ = 12. Next = 31 + 12 = **43**.
...
- Differences: 2, 3, 4, 5 → next = 6.
- Next position = 15 + 6 = **21 → letter U**.
...
**Solving it.** Step-by-step.
- Test Δ: 6, 12, 24, 48, ? → doubles each time. So next Δ = 96. Next term = 95 + 96 = **191**.
- Verify alternate rule: each term = 2 × prev + 1?
  - 2(5)+1 = 11 ✓; 2(11)+1 = 23 ✓; 2(23)+1 = 47 ✓; 2(47)+1 = 95 ✓; 2(95)+1 = **191** ✓.

***Watch out:*** "190" — students compute 95 × 2 = 190 and forget the +1.

**A similar question.** 1, 3, 7, 15, 31, ? → each = 2×prev + 1. Next = 2(31)+1 = **63**.

**Another twist — 2x − 1 pattern.** 3, 5, 9, 17, 33, ? → each = 2×prev − 1. Next = 2(33)−1 = **65**.
...
**Solving it.** 1=1², 4=2², 9=3², 16=4², 25=5². Next = 6² = **36**.

**A similar question — Square + 1.** 2, 5, 10, 17, 26, ? → n²+1. Next = 6²+1 = **37**.

**Another twist — Reverse squares.** 49, 36, 25, 16, 9, ? → 7², 6², 5², 4², 3², ? → next = 2² = **4**.
...
**Solving it.** Test relation between consecutive terms.
- 8 → 17: 8 × 2 + 1 = 17 ✓
- 17 → 35: 17 × 2 + 1 = 35 ✓
- 35 → 71: 35 × 2 + 1 = 71 ✓
- 71 → 143: 71 × 2 + 1 = 143 ✓
...
**Worth knowing:** Other "× a + b" patterns to test: ×2−1, ×3+2, ×3−1, etc.
...
**Solving it.**
- Odd positions (1st, 3rd, 5th, 7th): 2, 4, 6, 8 → AP with d = 2. Next: 10.
- Even positions (2nd, 4th, 6th, 8th): 5, 11, 17, 23 → AP with d = 6. Next: 29.
- 9th position is odd → next term = **10**.
...
**Solving it.** Decode letter and number separately.
- Letters: A(1), C(3), F(6), J(10), O(15) — diffs 2, 3, 4, 5 → next 6 → 15+6 = 21 → **U**.
- Numbers: 1, 4, 9, 16, 25 = squares → next = 36.
- Answer: **U36**.
...
> **Notation expansion:**
> **Coding** = applying a rule (shift, position, sum, reverse) to map letters → digits OR letters → letters.
> Always **write the alphabet row + position row first** on rough paper. This eliminates 80 % of errors.
> Common shift rules: +1, +2, +k; reverse (Z=1, A=26); position-dependent; block-shift.
...
**Step 1 — Extract mappings from given.**
- ROSE → 6821: R=6, O=8, S=2, E=1.
- CHAIR → 73456: C=7, H=3, A=4, I=5, R=6 (R confirms = 6 ✓).

**Step 2 — Look up each letter of SEARCH.**
...
**Solving it.** Step-by-step. Apply +1 to each letter of DONKEY.
- D → **E**
- O → **P**
- N → **O**
- K → **L**
...
**Another twist — Product instead of sum.** TEN = 20×5×14 = 1400.
...
**Solving it.** Step-by-step.
- 729 = 9³, 125 = 5³, 64 = 4³.
- For RED → R = ?, E = 5 ✓ (E=5 in alphabet, cube = 125 ✓), D = 4 ✓ (D=4, cube = 64 ✓).
- For R → 9, but R's alphabet position = 18, not 9. So mapping is **reverse position**: R → 27 − 18 = 9. Verify D: 27 − 4 = 23, but cube (D=4) gave 64 not 23³. Hmm — D's normal position 4 gives cube 64 ✓, so D uses normal position, not reverse.
...
**Solving it.** Step-by-step.
- PEN: P=16, E=5, N=14 → sum = 35 ✓ (matches).
- INK: I=9, N=14, K=11 → sum = 34 (NOT 38).
- Sum rule fails for INK. Try positions + 4? 34 + 4 = 38 ✓ — but PEN gave 35 (already correct without +4). Inconsistent rule.
...
- E(5) → G(7): +2; A(1) → F(6): +5; R(18) → T(20): +2; T(20) → V(22): +2; H(8) → H(8): 0?
- Re-check: maybe shifts are +2, +5, +2, +2, 0 — irregular. Try other rule.
- Position-based: shift = position × k. Position 1 shift +2, position 2 shift +5 (not multiple).
...
**Solving it.** Decode letter-by-letter.
- G→T: +13; O→B: +13 (O=15, +13 = 28 mod 26 = 2 = B); O→Y: +10? Inconsistent.
...
**Q10.7 (Advanced) — Chinese-coding (symbol mapping table).**
Mapping: cat = ⊕; dog = ⊗; fish = ▲; black = ◇; brown = ◆; quick = ★.
Phrase: "quick brown fish" coded as "★ ◆ ▲".
Find code for: "black cat".
...
- R × S: R is the brother of S. (So S is R's sibling — male or female; S has same parents as R, namely M and N.)
...
**Solving it.** Build the tree.
- A → B (B is A's child).
- B and C are brothers (same parents).
- D is E's mother.
- C married E → E is C's spouse.
- F is B's only child → F is B's child.
...
- F → C: nephew/niece. C → E: husband/wife. E → D: child. So D = mother of F's uncle's wife = mother of F's aunt-by-marriage.
...
**Q15.6 (Advanced) — Coded family with 4+ symbols.** Symbols: A + B = A is mother of B; A − B = A is brother of B; A × B = A is father of B; A ÷ B = A is son of B. Decode: P + Q − R × S ÷ T.
...
- S ÷ T: S is son of T.
- So: P is mother of Q (and Q is brother of R, hence P is also mother of R). R is father of S. T is father (or mother) of S — wait, S÷T means S is son of T, so T is parent of S. But R is father of S, so T must be S's mother (only one father possible). So T is wife of R.
...
- Distance from origin = √(3² + 0²) = **3 km** (due east of start).
...
**Another twist — Diagonal walk.** A walks 6 km N, then 8 km E. Distance from start = √(36 + 64) = √100 = **10 km** (a 6-8-10 Pythagorean triple).
...
**Solving it.** Step-by-step.
- Start: North (0°).
- Turn 135° clockwise → 0° + 135° = 135° (which is SE → S, more precisely SE→S between 135° pointing → SOUTH-EAST + 45° = SOUTH-EAST? Let me be careful: 0°=N, 45°=NE, 90°=E, 135°=SE, 180°=S, 225°=SW, 270°=W, 315°=NW). So 135° clockwise from N = **SE**.
- Turn 180° anticlockwise from SE: SE = 135°. Subtract 180° → −45° → +315° = **NW**.
...
- Distance AB = √[(5−0)² + (0−12)²] = √(25 + 144) = √169 = **13 km**.

**Worth knowing:** Pythagorean triple 5-12-13.

***Watch out:*** "17 km" — students add 12 + 5 linearly.

**A similar question — Triple 9-40-41.** A walks 9 km N, B walks 40 km E from same start. Distance = √(81 + 1600) = √1681 = **41 km**.
...
- Facing N, right side = East. Shadow east → light from west → sunset (evening).
...
**Solving it.** Track direction + position step by step.
...
- Distance from home = √(6² + 14²) = √(36 + 196) = √232 ≈ **15.23 km** in SE direction.
...
> **Notation expansion (used throughout):**
> Always **draw the seat skeleton first** (n cells in a row, or n-position circle). Place the most-constrained person first; permissive clues last.
> **"Immediate right/left"** = adjacent (1 seat away). **"Immediately between"** = exactly 1 cell apart.
> **Facing rule:** facing centre, "left" = clockwise; facing outward, "left" = anticlockwise.
...
- A > B > C → C is the lowest of A, B, C → C ∈ {1, 2}.
- D's exact floor — could be any of 1, 2, 3, 4. If D ≠ {A, B, C}, D takes 1 of 4 floors.
- For A > B > C to fit in 3 of the 4 floors (1, 2, 3, 4), C must be 1 or 2.
- If D = 4: A, B, C in {1, 2, 3} → A=3, B=2, C=1.
- If D = 1: A, B, C in {2, 3, 4} → A=4, B=3, C=2 (but then E=5, D=1, A=4 — D not below E necessarily? D=1 IS below E=5 ✓).
...
- Between positions 3 and 6: positions 4 and 5 → 2 people ✓.
...
> **Notation expansion (used throughout):**
> **Floor stacking:** floor 1 = ground (bottom), higher numbers = upper floors. "A above B" means A's floor number > B's floor number.
> **Days:** Mon, Tue, Wed, Thu, Fri, Sat, Sun. "X days after" is exclusive (not counting day 0).
> **Multi-attribute puzzle:** rows = persons; columns = attributes (city, sport, …). Build a table and fill in cells from clues.
...
- F is the shortest → F is below all.
- Combine: **D > C > A > B > E > F**.
...
- R = P − 5 = 2x − 5.
- Sum: x + 2x + (2x − 5) = 50.
- 5x − 5 = 50 → 5x = 55 → x = **11 years**.
- Verify: Q = 11, P = 22, R = 17. Sum = 11 + 22 + 17 = **50** ✓.

**A similar question.** A is thrice as old as B. C is 4 years older than B. Sum = 30. Find B. → B = x; A = 3x; C = x+4. Sum: x + 3x + x+4 = 30 → 5x = 26 → x = 5.2 (not whole — rare in exams; usually numbers chosen to give whole).

**Another twist — Average involved.** Average age of A, B, C = 25; A is twice B's age; C is 5 less than B. Sum = 75. 2B + B + (B−5) = 75 → 4B = 80 → B = 20. So A = 40, C = 15. Average = 75/3 = 25 ✓.
...
> **Notation expansion (used throughout):**
> **>** strict greater than (cannot be equal). **≥** greater than or equal to. Same for **<** and **≤**. **=** equal.
> **Chain rule:** A > B and B > C → A > C. But A > B and B = C → A > C (strict carries through). And A ≥ B and B > C → A > C (strict wins over ≥).
> **Inequality "breaks":** A > B and B < C tells us NOTHING about A vs C (the chain reverses direction at B).

---

**Q31.** Statements: A > B ≥ C; D ≤ B. Conclusions: (I) A > D, (II) D < C. Which conclusion(s) follows?
...
**Conclusion I: A > D?**
- We know A > B and D ≤ B → so D ≤ B < A → **D < A** is true → so **A > D** ✓.

**Conclusion II: D < C?**
- D ≤ B and B ≥ C → D ≤ B and C ≤ B. This tells us both D and C are below or equal to B, but does NOT order D vs C.
...
- > beats ≥ (i.e., A > B ≥ C → A > C strict).
- < beats ≤ (i.e., A < B ≤ C → A < C strict).
- ≥ ≥ = ≥ (cannot conclude strict).

***Watch out:*** Students conclude both follow because both "look" derivable. Always TRACE the chain — if there's a "≤" or "≥" you cannot turn into strict, conclusion is uncertain.

**A similar question — Both follow.** A > B > C ≥ D. Conclusions: (I) A > D (II) B > D. Both follow ✓ (chain doesn't break).

---

**Q32.** Statements: P > Q = R ≥ S. Conclusions: (I) P > S, (II) Q ≥ S.

**Solving it.**
- P > Q ≥ S → P > S (chain holds, > carries through ≥) ✓.
- Q = R and R ≥ S → Q ≥ S ✓.
- **Both conclusions follow.**

---

**Q33.** Statements: A ≥ B = C; D > A. Conclusions: (I) D > C, (II) A ≥ C.

**Solving it.**
- D > A and A ≥ C → D > A ≥ C → D > C ✓.
- A ≥ B and B = C → A ≥ C ✓.
- **Both conclusions follow.**
...
**Q33.5 (Advanced) — Coded inequality with 2 statements + 4 conclusions.**
**Symbols:** @ = >; # = ≥; & = <; $ = ≤; * = =.
**Statements:** P @ Q # R; S * T & R.
**Conclusions:** (I) P @ T (II) S & P (III) Q & T (IV) R @ S.

**Solving it.** Decode and analyse.
- Decoded: P > Q ≥ R; S = T < R.
- (I) P > T? P > Q ≥ R > T → P > T ✓.
- (II) S < P? S = T < R ≤ Q < P → S < P ✓.
- (III) Q < T? Q ≥ R > T → Q > T (not Q < T). ✗.
- (IV) R > S? S = T < R → R > S ✓.
- **Conclusions I, II, IV follow; III does not.**

---

**Q33.6 (Advanced) — Either/or conclusion.** Statements: A > B = C; D > B.
Conclusions: (I) A > D (II) D > A.
...
> **Notation expansion (used throughout):**
> **All A are B** = every A is B. **Some A are B** = at least one A is B. **No A is B** = no overlap.
> **Conversion rules:** "Some A are B" ⇔ "Some B are A" (always converts). "No A is B" ⇔ "No B is A" (always converts). "All A are B" does NOT convert — only "Some B are A" follows.
> **Universal method:** Draw Venn diagrams. Test each conclusion against ALL valid Venn arrangements.
...
**Step 1 — Visualise.**
- All cats are dogs → cats circle is INSIDE dogs circle.
- Some dogs are cows → dogs circle and cows circle OVERLAP (somewhere).

**Step 2 — Test Conclusion I.** "Some cats are cows."
- The cows-overlap with dogs could be anywhere — it might or might not include the cats subset.
- We cannot conclude that any cats are cows. → **Conclusion I does NOT follow**.

**Step 3 — Test Conclusion II.** "Some cows are dogs."
- This is the direct conversion of "Some dogs are cows" (which always holds for "some" statements).
- → **Conclusion II follows**.

**Answer: Only II follows.**

***Watch out:*** Students assume "if cats ⊂ dogs, and some dogs ∩ cows, then some cats ∩ cows." WRONG — the cow overlap might be entirely OUTSIDE the cats subset.
...
**Solving it.**
- No book is pen → books and pens DO NOT overlap.
- All pens are pencils → pens ⊂ pencils.
- So pens (= part of pencils) is disjoint from books → that part of pencils that overlaps with pens is definitely not books → **at least some pencils are not books** ✓.
- Conclusion II ("No pencil is book") — too strong. There MIGHT be other pencils (not pens) that overlap with books, since we have no info about non-pen pencils vs books. → does NOT follow strictly.
- **Only I follows.**
...
**Solving it.**
- Some doctors are professors → doctors ∩ professors ≠ empty.
- All professors are teachers → professors ⊂ teachers.
- So doctors ∩ professors ⊂ teachers. Hence those doctors are teachers → **Some doctors are teachers** ✓.
- By conversion of "Some doctors are teachers" → **Some teachers are doctors** ✓.
- **Both follow.**
...
**Solving it.**
- A ⊂ B and B ⊂ C → A ⊂ C → **All A are C** ✓.
- By conversion of "All A are C" → **Some C are A** ✓.
- **Both follow.**
...
**Solving it.**
- Some animals are pets → there exist some entities that are both animals AND pets.
...
**Q38.5 (Advanced) — 3 statements + 3 conclusions.**
Statements: All cars are vehicles. No vehicle is a fruit. Some fruits are red.
Conclusions: (I) No car is a fruit. (II) Some red things are not cars. (III) All cars are not fruits.

**Solving it.**
- Cars ⊂ Vehicles; Vehicles ∩ Fruits = ∅; Some Fruits ∩ Red.
- (I) Cars are vehicles; no vehicle is fruit → no car is fruit ✓.
- (II) Some fruits are red. Those red fruits are NOT cars (since fruits and cars are disjoint via vehicles). So some red things (= those fruit-reds) are not cars ✓.
- (III) "All cars are not fruits" = "No cars are fruits" = same as I ✓.
- **All three follow.**

---

**Q38.6 (Advanced) — Possibility conclusion (Banks Mains).**
Statements: Some pens are pencils. All pencils are erasers.
Conclusions: (I) All pens being erasers is a possibility. (II) Some erasers are not pens.

**Solving it.**
- (I) Pens overlap with pencils. Pencils are inside erasers. The portion of pens NOT in pencils could ALSO be inside erasers (no statement forbids it). So "all pens being erasers" is POSSIBLE. ✓.
- (II) Erasers contain all pencils PLUS possibly more. The "more" portion may or may not include the non-pen pens. Cannot conclude DEFINITELY some erasers are not pens. (Could be all erasers ARE pens in one valid Venn.) ✗.
- **Only I follows.**
...
> **Notation expansion (used throughout):**
> Banks Mains specialty. A "machine" applies a fixed rule to an input list, producing successive steps. The candidate must IDENTIFY the rule from input → step 1, then predict later steps.
> **Universal first move:** Compare INPUT with STEP 1 element-by-element. The rule will be visible in 30 seconds with practice.

---

**Q39.** Input: 12 24 6 18 9. Step 1: 6 12 24 18 9. Find the rule.

**Solving it.** Compare position-by-position.
- Position 1: 12 → 6. Position 2: 24 → 12. Position 3: 6 → 24. Position 4-5: unchanged (18, 9).
...
**Q41.5 (Advanced) — 2-rule machine.** Input: 24 17 35 12 48 9.
Step 1: 9 24 17 35 12 48.
Step 2: 9 12 24 17 35 48.
...
**Q41.6 (Advanced) — Symbol-shift rule.** Input: A1 B5 C3 D7 E2.
Step 1: B5 A1 C3 D7 E2 (largest letter+number combo moved to front).
...
> **Notation expansion (used throughout):**
> Question + 2 statements (I, II). Decide which subset of statements is sufficient to solve.
> **Standard answer choices:**
> (a) Statement I alone is sufficient.
> (b) Statement II alone is sufficient.
> (c) Each statement ALONE is sufficient.
> (d) BOTH statements together are needed (neither alone suffices).
> (e) Neither alone nor together is sufficient.
> **Discipline:** test each statement IN ISOLATION. Do not look at one while testing the other.

---

**Q42.** Find the value of x.
Statement I: x + 5 = 12.
Statement II: x − 3 = 4.

**Solving it.**
- I alone: x = 12 − 5 = **7** → I sufficient.
- II alone: x = 4 + 3 = **7** → II sufficient.
- **Each alone is sufficient. Answer: (c).**

---

**Q43.** Is x an even number?
Statement I: x = 4y, where y is a positive integer.
Statement II: x is divisible by 3.

**Solving it.**
- I alone: 4y = 4 × (positive integer) = always EVEN (multiple of 4 is always multiple of 2). → Yes, x is even. **I sufficient.**
- II alone: divisible by 3 → x could be 3 (odd), 6 (even), 9 (odd), 12 (even), … → cannot decide parity. **II not sufficient.**
- **Only I alone is sufficient. Answer: (a).**

---

**Q44.** Find the perimeter of a rectangle.
Statement I: Length = 10 cm.
Statement II: Area = 30 cm².

**Solving it.**
- I alone: Length given but no width → cannot find perimeter. **I not sufficient.**
- II alone: Area 30 = L × W → infinite (L, W) pairs (1 × 30, 2 × 15, 3 × 10, …) → cannot find perimeter. **II not sufficient.**
- I + II together: L = 10, Area = 30 → W = 30 / 10 = 3 → Perimeter = 2(10 + 3) = **26 cm**. ✓
...
**Q44.5 (Advanced) — 3-statement DS.** Find the age of A.
(I) A is 5 years older than B.
(II) B is 25 years old.
(III) A + B = 55.

**Solving it.**
- I + II → A = B + 5 = 30. Sufficient.
- I + III → 2B + 5 = 55 → B = 25, A = 30. Sufficient.
- II + III → A = 55 − 25 = 30. Sufficient.
- **Any TWO statements together suffice. Single statement alone does NOT.**

---

**Q44.6 (Advanced) — Quant DS with conditions.** Is x positive?
(I) x² > 4.
(II) x³ > 0.

**Solving it.**
- I: x² > 4 → x > 2 or x < −2. Could be positive OR negative. Insufficient alone.
- II: x³ > 0 → x > 0. Sufficient alone.
...
**Solving it.** Step-by-step.
- **Step 1 — Reverse the order of letters.** INDIA → AIDNI.
- **Step 2 — Mirror each glyph.** Some letters look the same in mirror (A, H, I, M, O, T, U, V, W, X, Y); others flip visibly (B, C, D, E, K, etc.).
- For INDIA: A → A; I → I; D → ᗡ (D-reversed); N → И (N-reversed); I → I.
- Final visible answer: **A I ᗡ И I** (read left-to-right as AIDNI with D and N glyph-flipped).
...
**A similar question — Water image of "BOAT".** Vertical flip of each letter. B mostly looks the same (top-bottom roughly symmetric); O symmetric; A flips upside-down (∀); T inverts to ⊥. Result: B O ∀ ⊥.

---

**Q46.** A cube of side 4 cm is painted on all 6 faces, then cut into 1 cm³ small cubes. How many small cubes have exactly 1 face painted?

**What kind of problem is this?** Painted-cube counting. Use the standard formula.

**Solving it.** Step-by-step.
- n = 4 (cube cut 4 × 4 × 4 = 64 small cubes).
- Face-centre small cubes (1 face painted) = **6 × (n − 2)² = 6 × (4 − 2)² = 6 × 4 = 24**.

**Worth knowing:** Memorise the painted-cube formulas cold.

| Type | Count for n × n × n cube |
|---|---|
| Corner (3 faces painted) | always 8 |
| Edge (2 faces painted) | 12(n − 2) |
| Face-centre (1 face painted) | 6(n − 2)² |
| Interior (0 faces painted) | (n − 2)³ |
| Total | n³ |

- Verify for n = 4: 8 + 12×2 + 6×4 + 2³ = 8 + 24 + 24 + 8 = **64** = 4³ ✓.

**A similar question — Cube n=5, find 0-painted.** Interior = (5−2)³ = 27.

**Another twist — n=6, find 2-painted.** Edge = 12 × 4 = 48.
...
**Solving it.**
- Fold along diagonal → triangular shape.
- Cut a triangle at centre → on unfolding, the cut REFLECTS across the diagonal.
...
> **Notation expansion (used throughout):**
> **kth from left** in a row of n = position k. **kth from right** = position (n − k + 1).
> **Total when both ranks given:** n = (left-rank) + (right-rank) − 1 (subtract 1 because the person is counted in both).
> **After moving:** if A's left-rank changes from k to k', A moved (k' − k) positions to the right.
...
**Solving it.** Compare letter-by-letter.
...
**Input:** 25 38 14 47 19 56.
**Step 1:** 14 25 38 19 47 56.
**Step 2:** 14 19 25 38 47 56.
...
(II) Some erasers are definitely pens.

**Solving it.**
- Some pens ∩ pencils → those overlap-pens are also erasers (since all pencils are erasers).
...
A man starts walking facing North. He walks 5 km, then turns 45° clockwise and walks 7√2 km, then turns 90° anticlockwise and walks 3 km. Find his final position relative to start.

**Solving it.**
- Start: facing N, position (0, 0).
- Walk 5 km N → position (0, 5).
- Turn 45° clockwise → now facing NE. Walk 7√2 km in NE → moves equally in N and E: 7√2 × cos 45° = 7 in N, 7 in E. Position: (7, 12).
- Turn 90° anticlockwise from NE → now facing NW. Walk 3 km NW: −3 cos 45° = −3/√2 ≈ −2.12 in E, +3/√2 ≈ +2.12 in N. Position: (7 − 2.12, 12 + 2.12) = (4.88, 14.12).
- Distance from start = √(4.88² + 14.12²) ≈ √(23.8 + 199.4) = √223.2 ≈ **14.94 km**, direction ≈ NNE.
...
| Cube — corner vs edge vs face count | Painted cube | Memorise: corners 8, edges 12(n−2), faces 6(n−2)², interior (n−2)³ |
...
*Method.* Compare element-by-element.
- 6 = smallest; was at position 3, now at position 1.
- 12, 18, 24 = sorted ascending; were at positions 1, 4, 2.
- 9 stays at position 5.
...
*Method.* (II) + (I) → X = 35 (sufficient). (III) + (I) → 2Y + 5 = 65 → Y = 30 → X = 35 (sufficient). (II) + (III) → Y = 30 → X = 35 (sufficient). Any TWO suffice. Best answer: any two of I, II, III.
...
*Method.* (I) is "if and only if" — statement says "8+ hr → passed", not the converse. (I) doesn't follow. (II) statement doesn't say anything about <8 hr students. (II) doesn't follow strictly. Neither follows.
...
*Method.* (I) follows from "some ministers spoke" ⊂ "all attended". (II) follows from "ministers spoke" ⊂ "all who spoke" — only if no non-minister also spoke (statement doesn't say). So (II) doesn't strictly follow. Only (I).
...
**Q7.** Should reservation in education be abolished?
- Argument 1 (Yes): "Equality of opportunity is a constitutional principle."
- Argument 2 (No): "Centuries of injustice need correction; affirmative action provides it."

*Method.* Both are STRONG (relevant to question; substantial). The answer is "both arguments are strong".

**Q8.** Should children be given homework?
- Yes: "Homework reinforces learning."
- No: "Children also need play time."

Both arguments are strong + valid; both follow.

**Q9.** Should the voting age be lowered to 16?
- Yes: "16-year-olds are aware of issues today."
- No: "16 is too young for political maturity."

Both have merit; both follow.

## H.4 Cause & Effect (3)

**Q10.** Statement A: "Inflation rose to 8%." Statement B: "RBI raised interest rates."
- A could be cause; B could be effect (RBI responds to inflation by raising rates).
- B → A is also possible (rates affect inflation with lag).
- Most likely: A is cause, B is effect (in the immediate term).

**Q11.** A: "Heavy rain forecast." B: "Schools declared closed."
- A → B (schools close due to rain forecast).

**Q12.** A: "PM announced new healthcare scheme." B: "Stock market rose."
- Loose correlation, no clear causation. "Independent events".
...
**Cube formula** (n × n × n cube painted then cut to 1 cm³):
- Corners: 8 (3-painted)
- Edges: 12(n−2) (2-painted)
- Faces: 6(n−2)² (1-painted)
- Interior: (n−2)³ (0-painted)

**Inequality chain** breaks at sign reversal. > beats ≥ beats =.
